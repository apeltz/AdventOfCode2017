 ...
 *
 * @param {String} digitString  Integers separated by tabs and newline characters, where each newline denotes the end of a row
 ...
 * // => 18
 */
function checkSumReducer(digitString) {
	return digitString.split('\n').reduce((result, row) => result += getDiff(row.split('\t')).diff, 0);
}
 ...
 */
function getDiffReducer(arr) {
	return arr.reduce((mm, n) => {
		mm.min = !mm.min || +n < mm.min ? +n : mm.min;
		mm.max = !mm.max || +n > mm.max ? +n : mm.max;
		mm.diff = mm.max - mm.min;
		return mm;
	}, {});
}

results.checkSumReducer = checkSumReducer(digitString);
