const digitString = `una bokpr ftz ryw nau yknf fguaczl anu
...
function validPassphraseCount(passPhrases, cb) {
	return passPhrases.split('\n').reduce((result, row) => result += cb(row.split(' ')), 0);
}

function noDuplicates(passPhrasesArr) {
	let phrases = {}
	for (let i = 0; i < passPhrasesArr.length; i++) {
		if (phrases[passPhrasesArr[i]]) return 0;
		phrases[passPhrasesArr[i]] = true;
	}
	return 1;
}

function noAnagrams(passPhrasesArr) {
	let phrases = {}
	for (let i = 0; i < passPhrasesArr.length; i++) {
		let anagramId = createAnagramId(passPhrasesArr[i]);
		if (phrases[anagramId]) return 0;
		phrases[anagramId] = true;
	}
	return 1;
}

function createAnagramId(str) {
	let result = '0'.repeat(125) // ASCII chars
	for (let i=0; i<str.length; i++){
		let idx = str.charCodeAt(i);
		let incremented = Number(result[idx]) + 1;
		result = result.substr(0, idx) + incremented + result.substr(idx+1);
	}
	return result;
}

let result = validPassphraseCount(digitString, noAnagrams);

module.exports = {
	validPassphraseCount: validPassphraseCount,
